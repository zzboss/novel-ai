<template>
  <el-dialog
    ...
    :title="isModifyMode ? 'AI 修改灵感' : 'AI 生成灵感'"
    width="700px"
  ...
  >
    ...
    <template #header>
      <div class="dialog-header">
        <span class="dialog-title">{{ isModifyMode ? 'AI 修改灵感' : 'AI 生成灵感' }}</span>
        <el-button
          text
    ...
    <div class="input-section mb-4">
      <label class="input-label">
        {{ isModifyMode ? '请输入修改要求：' : '请输入灵感描述或关键词：' }}
      </label>
      <el-input
        v-model="userInput"
        type="textarea"
        :autosize="{ minRows: 6, maxRows: 12 }"
        :placeholder="isModifyMode ? '例如：增加一些悬疑元素，让故事更紧张...' : '例如：一个关于时间旅行的科幻故事，主角是一名历史学家...'"
    ...
    <div class="action-section mb-4">
      ...
      >
        {{ isGenerating ? '生成中...' : (isModifyMode ? '修改灵感' : '生成灵感') }}
      </el-button>
      <el-button
    ...
    <div v-if="isGenerating" class="streaming-section mb-4">
      ...
      />
      <p class="generating-text mt-2">⏳ {{ isModifyMode ? '正在修改灵感...' : '正在生成灵感...' }}</p>
    </div>
    ...
    <div v-if="!generatedContent && !isGenerating" class="tips-section mt-4">
      <el-alert type="info" :closable="false">
        ...
        <ul class="tips-list">
          ...
          <li v-if="!isModifyMode">可以包含：题材、风格、主题、核心冲突等信息</li>
          <li v-if="!isModifyMode">也可以只输入简单关键词，AI会补充细节</li>
          <li v-if="isModifyMode">描述你希望如何修改当前内容</li>
          <li v-if="isModifyMode">可以指定修改特定部分，或整体调整风格</li>
        </ul>
      </el-alert>
    </div>
...
async function handleGenerate(): Promise<void> {
  if (!userInput.value.trim()) {
    ElMessage.warning(isModifyMode.value ? '请输入修改要求' : '请输入灵感描述')
    return
  }
  ...
  try {
    ...
    if (isModifyMode.value) {
      // 修改模式
      result = await agentStore.modifyIdea(
      ...
      // 生成模式
      result = await agentStore.generateIdea(
        userInput.value,
        (props.projectType as any) || 'novel',
        undefined,
        selectedModelConfig.value
    ...
    ElMessage.success(isModifyMode.value ? '灵感修改成功！' : '灵感生成成功！')
    
  } catch (error: any) {
    ElMessage.error((isModifyMode.value ? '修改' : '生成') + '失败：' + (error.message || '未知错误'))
    console.error((isModifyMode.value ? '修改' : '生成') + '灵感失败：', error)
  } finally {
